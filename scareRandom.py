...
video_prefix = ["Viv", "Coffin", "Ghoul", "Zombie", "Onlytwo", "Dad"] # This is the list of videos prefixes, you can add additional video
# prefixes here.
#video = ["omxplayer", "filename", "-o", "both", "--win", "0 0 1280 720", "--aspect-mode", "fill", "--no-osd", "--orientation", "180", "--vol", "-600"]
video = ["omxplayer", "filename", "--align", "center", "--aspect-mode", "fill", "--no-osd", "--orientation" ,"180", "--vol", "-600"]
...
def change_video():
    ...
    if elapsed_time > (int(sys.argv[2]) * 60):
        ...
        scare_file = "/home/pi/Projects/Halloween/ScareMedia/{0}Scare.mp4".format(current_prefix)
        start_time = time.time()
...
def show_image(_image_name):
    os.system("sudo fbi -a -T 1 -d /dev/fb0 -noverbose -once /home/pi/Projects/Halloween/ScareMedia/{0}Start.png".format(_image_name))
...
def start_up():
    ...
    global image_name
    image_name = arg1
    scare_file = "/home/pi/Projects/Halloween/ScareMedia/{0}Scare.mp4".format(image_name)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if arg1 not in video_prefix:
            raise ValueError('first argument must be [Videoname] without Start or Scare')
        if sys.argv[2].isdigit():
            arg2 = int(sys.argv[2])
        else:
            raise ValueError('Second argument must be a number [in minutes]')
    except IndexError:
